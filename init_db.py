...
def init_database():
    ...
    try:
        ...
        # Backfill gallery schema for existing databases.
        cursor.execute("""
            SELECT COLUMN_NAME
            FROM information_schema.COLUMNS
            WHERE TABLE_SCHEMA = %s AND TABLE_NAME = 'gallery'
        """, (DB_CONFIG['database'],))
        gallery_columns = {row[0] for row in cursor.fetchall()}
        if 'image_url' not in gallery_columns:
        ...
        if hasattr(e, 'errno') and e.errno == 1045:
            print("ERROR: Authentication error connecting to MySQL. Check DB_USER/DB_PASSWORD in your environment or .env file.")
